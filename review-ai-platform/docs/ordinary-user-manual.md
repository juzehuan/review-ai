...
分析完成后，打开任务报告查看 AI 输出结果。新版报告页顶部会显示任务名称、来源、分析类型、生成时间和导出入口，正文会先给出执行摘要，再展示关键指标和详细洞察。
...
1. 先看执行摘要，判断整体态度、主要问题和优先关注点。
2. 查看报告快照，确认评论样本、正向占比、负向或争议评论、观点支持度等关键指标。
3. 查看 NPS、情绪分布、来源分布、意图分布和问题统计，理解评论结构。
4. 查看用户声音、高频词和主题词，确认评论主要讨论什么。
5. 查看痛点、机会点和行动建议，判断哪些问题值得优先处理。
6. 回到评论明细抽查原文，确认报告结论有原始评论支撑。

导出报告：

- 点击报告页右上角“导出报告”。
- 选择“导出 Markdown”用于内部编辑或知识库沉淀。
- 选择“导出 HTML”用于静态页面归档或发送给同事查看。
- 选择“打印 / 保存 PDF”用于正式汇报或留档。
