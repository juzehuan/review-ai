...
分析完成后，进入任务报告页查看 AI 输出结果。新版报告页顶部会显示当前报告信息、任务来源、分析类型、生成时间和操作入口，正文先给出执行摘要，再展示关键指标快照、主题洞察、用户声音、代表性评论、痛点、机会点和行动建议。

![分析报告与导出入口](assets/user-manual/12-analysis-report.png)

报告阅读顺序建议：

1. 先看执行摘要，快速判断整体态度、主要矛盾和优先关注点。
2. 查看报告快照，确认评论样本、正向占比、负向或争议评论、观点支持度等关键指标。
3. 查看情感分布、来源分布、意图分布、问题统计和动态标签，理解评论结构。
4. 查看用户声音、高频词和观点聚类，判断用户真正集中讨论什么。
5. 查看痛点、机会点和行动建议，决定后续运营、内容或产品动作。
6. 进入评论明细，抽样核对原始评论是否支持结论。

报告导出方式：

- 导出 Markdown：适合内部复盘、二次编辑或粘贴到知识库。
- 导出 HTML：适合以静态页面方式打开、归档或发送给团队成员查看。
- 打印 / 保存 PDF：调用浏览器打印功能，适合正式汇报或留档。

导出前建议先复核代表性评论，确认不存在敏感信息、个人隐私或不适合外发的原文。图表和页面交互以系统页面为准，导出文件会保留结构化结论、指标和证据文本。
