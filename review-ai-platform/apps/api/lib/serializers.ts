...
export function serializeCrawlJob(job: CrawlJobWithWorkspace): CrawlJobDTO {
  ...
  return {
    ...
    idleRounds: readOptionalNumber(rawResult?.idleRounds),
    lastAddedRows: readOptionalNumber(rawResult?.lastAddedRows),
    noMoreButtonRounds: readOptionalNumber(rawResult?.noMoreButtonRounds),
    lastLoadMoreClicked: readOptionalBoolean(rawResult?.lastLoadMoreClicked),
    cursor: readOptionalScalarString(rawResult?.cursor),
    hasMore: readOptionalBoolean(rawResult?.hasMore),
    lastRequestStatus: readOptionalNumber(rawResult?.lastRequestStatus),
    totalComments,
